...
class TestOperator(TestCase):
    ...
    def test_individual_barg_operators(self, barg_index: int):
        ...
        def int_to_ternary(value: int) -> Tuple[int, int]:
            ...
            Raises:
                QiskitError: if ``value`` is not a two-trit number.
            """
        ...
        expected_mat = np.linalg.matrix_power(barg_x, i) @ np.linalg.matrix_power(
            barg_z, j
        )

        self.assertTrue(
            np.array_equal(expected_mat, actual_mat),
            f"Actual Barg operator matrix is not as expected. Expected\n{expected_mat}\n, instead got\n{actual_mat}.",
        )
